...
def test_core_does_not_import_persistence_adapters_or_sqlite_libraries() -> None:
    violations: list[tuple[Path, str]] = []
    forbidden = {"aiosqlite", "alembic", "sqlalchemy", "sqlite3"}
    for root in (Path("src/revio/domain"), Path("src/revio/application"), Path("src/revio/ports")):
        for path in root.rglob("*.py"):
            tree = ast.parse(path.read_text())
            for node in ast.walk(tree):
                ...
                violations.extend(
                    ...
                    or name == "revio.adapters.persistence"
                    or name.startswith("revio.adapters.persistence.")
                )
    assert violations == []
